...
namespace TwinSunsBot.Net
{
   static public class Sneaky
        {
            private static string text = File.ReadAllText(@"c:\important\discordtoken.txt", Encoding.UTF8); // Visual Studio keeps uploading Sneaky.css... so we'll load the token from a file that Visual Studio can't see.
